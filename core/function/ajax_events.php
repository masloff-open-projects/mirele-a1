...
function MAjax()
{
    ...
    if (!get_option('mirele_forbid_updates', true)) {
        add_action('wp_ajax_upgrade_components', function () {
            ...
            $return = MAccount::execute(function () {
                ...
                if (MBackup::create(get_template_directory(), true)) {
                    ...
                    $mupgrade->upgrade_components(MRepository::components_versions());

                    return array(
                        'errors' => $mupgrade->getErrors(),
                        'success' => $mupgrade->getSuccess(),
                    );

                } else {
                    return array(
                        'errors' => [
                            'The system could not create a backup.'
                        ],
                        'success' => '',
                    );
                }
