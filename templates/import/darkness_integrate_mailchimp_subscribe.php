...
/**
 * Rosemary Template: Darkness MailChimp Subscribe;
 */

if (MIRELE_INTEGRATION_MAILCHIMP) {

    rosemary_register('darkness_mailchimp_subscribe', function ($event=null) {
    ...
    <section class="theme theme-dark-no-full-height text-center">
        ...
        <form method="POST" list_id="<?php echo $options->mailchimp_list ?>" action="subscribe_to_news_mailchimp_form">
            ...
            <?php foreach (['email', 'phone', 'lname', 'fname'] as $input): ?>
                ...
                <div>
                    <input type="<?php echo $input == 'email' ? 'email' : 'text' ?>" name="<?php echo $input; ?>" placeholder="<?php echo $input_ ?>" class="darkness-input margin-x-md">
                </div>
    ...
    <?php }, array(
        'title' => 'Darkness MailChimp Subscribe Integrate',
        ...
        'author' => 'Mirele Package Darkness Integrate'
    )); 
