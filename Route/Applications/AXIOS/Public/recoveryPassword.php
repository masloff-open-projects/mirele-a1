...
/**
 * Class Request_recoveryPassword
 * @package Mirele\Network
 * @alias recoveryPassword
 * @description The Endpoint serves to recovery password
 * @version 1.0.0
 */
class Request_recoveryPassword extends Request
{
        ...
        } else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    $rp_link = "<a href='".wp_login_url(
                        )."?action=rp&key=$adt_rp_key&login=".rawurlencode($user->user_login)."'>".wp_login_url(
                               )."?action=rp&key=$adt_rp_key&login=".rawurlencode($user->user_login)."</a>";

                    $message = "Hello dear user! <br>";
                    $message .= "An account has been created on ".get_bloginfo('name'
                        )." for email address ".$_POST['email']."<br>";
