...
$(function () {
        ...
        $(document).ready(function () {
            if ($('#upgrade').length !== 0) {
                $("#upgrade").submit(function (e) {
                    e.preventDefault();
                    mirele_auth_bridge ('upgrade_components', function (response) {

                        if ('errors' in response && response.errors.length == 0) {

                            $.toast('Components are successfully updated');
    ...
    /**
     * Spotlight for quick work with resource
     *
     * @author Mirele
     * @version 1.0.0
     */

    karlin(document).ready(function () {
        karlin(document).keyup(
            function () {
                karlin("body").spotlight(function (e) {
                    param = e;

                    switch (e.toLowerCase().replace(/\s/g, "")) {
                        case "gocenter":
                            location.search = "?page=mirele_center";
                            break;

                        case "gopages":
                            location.search = "?page=rosemary_render_pages_list";
                            break;

                        case "gowoost":
                            location.search = "?page=mirele_center&tab=woocommerce";
                            break;

                        case "gowpst":
                            location.search = "?page=mirele_center&tab=wptheme";
                            break;

                        case "inblock":
                            location.search =
                                "?page=rosemary_render_blocks_manager&tab=install";
                            break;

                        case "cb":
                            $('[ data-action="create_block"]').click();
                            break;

                        case "rab":
                            $('[ data-action="remove_block"]').click();
                            break;

                        case (e.match(/page/) || {}).input:
                            location.search =
                                "?page=rosemary_render_editor_page&page_id=" +
                                String(param).replace("page", "");
                            break;

                        case (e.match(/binstall/) || {}).input:
                            rosemary_install(String(param).replace("binstall", ""));
                            break;
